...
def tail_stats():
    ...
    with eng.connect() as c:
        q = c.execute(text("SELECT count(*) AS n, sum(bust_multiplier>=2.0) AS ge2, sum(bust_multiplier>=10.0) AS ge10, sum(bust_multiplier>=50.0) AS ge50 FROM rounds"))
        row = q.first()
    n = row.n or 0
    def freq(x): 
...
def recent(limit: int = 100):
    eng = get_engine()
    with eng.connect() as c:
        q = c.execute(text("SELECT round_id, timestamp, bust_multiplier FROM rounds ORDER BY id DESC LIMIT :lim"), {"lim": limit})
